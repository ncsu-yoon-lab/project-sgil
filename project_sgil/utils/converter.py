...
class Converter:
    ...
    Provides methods for converting between latitude/longitude and Cartesian coordinates, as well as
    image x-coordinate to angle and compass heading to yaw.
    """
    ...
    def image_x_to_theta(x: float, image_width: int = 1280) -> float:
        ...
        Converts horizontal pixel offset to an angle using half the horizontal field of view
        (H_FOV_DEG).
    ...
    def heading_to_yaw(heading: float) -> float:
        ...
        Yaw is measured by finding heading pointing the x direction (parallel to vector from EB1 to
        EB3).
