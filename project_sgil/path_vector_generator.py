...
class PathVectorGenerator:
    ...
    def get_intersection(
        ...
        if left_points[0][0] == left_points[1][0]:
            ...
            m_left = (left_points[0][1] - left_points[1][1]) / (
                left_points[0][0] - left_points[1][0]
            )
        if right_points[0][0] == right_points[1][0]:
            m_right = 9999.0
        else:
            m_right = (right_points[0][1] - right_points[1][1]) / (
                right_points[0][0] - right_points[1][0]
            )

        # Gets the y-intercept of the left and right lines
        b_left = left_points[0][1] - m_left * left_points[0][0]
        b_right = right_points[0][1] - m_right * right_points[0][0]

        # Gets the x and y intersection point
        x_int = (b_left - b_right) / (m_right - m_left)
        y_int = m_left * x_int + b_left

        return (x_int, y_int), m_left, m_right
    ...
    def get_closest_point_along_line(
        ...
        m = 9999 if x1 == x2 else (y2 - y1) / (x2 - x1)
        m_inv = -1 / m
        ...
        dist = math.sqrt((x_int - x3) ** 2 + (y_int - y3) ** 2)
    ...
    def get_path_data(
        ...
        estimated_pose_vector = (
            math.cos(math.radians(estimated_heading_deg)),
        ...
        delta_heading_prev_node = math.acos(
            (
                estimated_pose_vector[0] * prev_node_vector[0]
                + estimated_pose_vector[1] * prev_node_vector[1]
            )
            / prev_node_vector_dist
        )
        delta_heading_cur_node = math.acos(
            (
                estimated_pose_vector[0] * cur_node_vector[0]
                + estimated_pose_vector[1] * cur_node_vector[1]
            )
    ...
    def get_path_vector(
        self,
        start_node_xy: tuple[float, float],
        ...
        path_dist = math.sqrt(
            (end_node_xy[0] - start_node_xy[0]) ** 2 + (end_node_xy[1] - end_node_xy[0]) ** 2
        )
        path_unit_vector = (
            (end_node_xy[0] - start_node_xy[0]) / path_dist,
            (end_node_xy[1] - start_node_xy[1]) / path_dist,
        )
        ...
        transformed_end_node_xy = (
            end_node_xy[0] + inv_path_unit_vector[0] * abs(displacement),
        ...
        while True:
            ...
            if key == 13:  # Enter key
                ...
                if len(self.points) == 4:
                    ...
                    # Round the intersection point for when it is displayed
