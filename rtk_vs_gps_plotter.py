...
class GPSRTKVisualizer:
    ...
    def initialize_plot(self):
        ...
        self.rtk_scatter = self.ax.scatter(rtk_lons, rtk_lats, color='red', alpha=0.5, label='RTK Path')
        
        # Plot all GPS points
        gps_lats = self.data['gps_lat'].values
        gps_lons = self.data['gps_lon'].values
        self.gps_scatter = self.ax.scatter(gps_lons, gps_lats, color='blue', alpha=0.5, label='GPS Path')
        ...
        self.current_rtk_point = self.ax.scatter([], [], color='darkred', s=100, label='Current RTK')
        self.current_gps_point = self.ax.scatter([], [], color='darkblue', s=100, label='Current GPS')
    ...
    def update_plot(self):
        ...
        timestamp_info = f"Time: {current_data['timestamp']:.2f}" if 'timestamp' in current_data else ""
        self.ax.set_title(f'GPS vs RTK Position (Point {self.current_index+1}/{len(self.data)}) - Error: {error_m:.2f}m {timestamp_info}')
    ...
    def create_interactive_map(self, output_file='gps_rtk_map.html'):
        ...
        # Add RTK path
        rtk_coords = [(lat, lon) for lat, lon in zip(self.data['rtk_lat'], self.data['rtk_lon'])]
        folium.PolyLine(rtk_coords, color='red', weight=3, opacity=0.7, tooltip='RTK Path').add_to(m)
        
        # Add GPS path
        gps_coords = [(lat, lon) for lat, lon in zip(self.data['gps_lat'], self.data['gps_lon'])]
        folium.PolyLine(gps_coords, color='blue', weight=3, opacity=0.7, tooltip='GPS Path').add_to(m)
        
        # Add markers for each point with popups showing the error
        for i, row in self.data.iterrows():
            # Calculate error
            error_m = self.calculate_haversine_distance(row['rtk_lat'], row['rtk_lon'], row['gps_lat'], row['gps_lon'])
            
            # Add RTK marker
            rtk_popup_text = f"RTK Point {i+1}<br>Lat: {row['rtk_lat']:.8f}<br>Lon: {row['rtk_lon']:.8f}"
            if 'timestamp' in row:
    ...
    def run(self):
        ...
        print("Visualization complete. Press Enter to advance, Backspace to go back, or Escape to close.")
